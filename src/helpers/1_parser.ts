...
import type { JoinInfo, ParsedQuery, WhereCondition } from "./types";
...
export function parseSqlQuery(input: string) {
    ...
    try {
        ...
        const { mainTable, activeTables, joins } =
            parseTablesAndJoins(rawTables);
...
function validateAttributes(
    ...
    if (attParts.length === 1) {
        const exists = activeTables.some((table) =>
            isAttributeInTable(table, att),
        );
